...
export function AchievementBadge({
  ...
  const textColor = isLocked ? 'text-white/40' : 'text-white';
  const borderColor = isEarned ? 'border-brand' : 'border-white/20';

  return (
    <View className={`bg-white/10 rounded-2xl p-4 border ${borderColor} ${opacity}`}>
      <View className="flex-row items-start gap-3">
        ...
        <View className="flex-1">
          ...
          <Text className="text-white/60 text-xs mt-1">{achievement.description}</Text>

          {!isEarned && progress > 0 && progress < 100 && (
            <View className="mt-2 bg-white/10 rounded-full h-1.5 overflow-hidden">
              <View
          ...
          {isLocked && progress === 0 && (
            <Text className="text-white/40 text-xs mt-2">Bloqueado</Text>
          )}
        </View>
      </View>
    </View>
