...
function DebtBreakdownItem({
  ...
  return (
    <View className="flex-row items-center justify-between py-1 px-2">
      <Text className="text-fg/60 text-xs flex-1">{productName}</Text>
      <Text className="text-fg/60 text-xs">{quantity}x</Text>
      <Text className="text-fg text-xs font-semibold ml-2">{amount.toFixed(2)}€</Text>
    </View>
  );
}
...
export function MemberDebt({ debt, profile, onPress }: MemberDebtProps): React.ReactElement {
  ...
  const statusColor = debt.isPaid ? 'text-fg/40' : 'text-brand';
    ...
    >
      <View className="flex-row items-center justify-between mb-2">
        <View className="flex-1">
          <Text className="text-fg font-bold">{profile?.name || 'Membro'}</Text>
      ...
      {debt.breakdown.length > 0 && (
        <View className="mt-2 border-t border-fg/10 pt-2">
          <FlatList
            data={debt.breakdown}
      ...
      <Text className="text-fg/40 text-xs mt-2">{debt.totalDrinks} bebidas</Text>
    </Pressable>
  );
}
